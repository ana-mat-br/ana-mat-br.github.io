...
\subsection{Como obter os bancos}\label{como-obter-os-bancos}
...
\textbf{A mais rápida: nem baixe.} O R lê um arquivo direto da internet, bastando dar o endereço no lugar do nome do arquivo. Copie a linha, cole no seu script e execute:
...
\textbf{A segunda: baixe o arquivo.} Clique no link da tabela acima, salve o arquivo dentro da pasta do seu projeto do RStudio, em uma subpasta chamada \texttt{dados}. Aí o caminho fica curto:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{nascimentos }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"dados/nascimentos.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Esta é a forma que usaremos daqui em diante, porque é a que você vai precisar quando estiver analisando os \textbf{seus} dados, que não estarão em endereço nenhum da internet. Se você preferir, também pode usar o botão \textbf{Import Dataset} que aprendemos no começo do capítulo --- ele faz exatamente a mesma coisa e escreve o código para você.

\begin{quote}
\textbf{Dica:} se aparecer a mensagem \texttt{does\ not\ exist\ in\ current\ working\ directory}, o R não achou o arquivo. Confira se você está dentro do Projeto certo (canto superior direito do RStudio) e se a pasta \texttt{dados} está no mesmo lugar que o seu arquivo \texttt{.Rproj}.
\end{quote}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{saude }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"dados/saude{-}mental.csv"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aptidao }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"dados/aptidao{-}fisica.csv"}\NormalTok{)}
