...
\section{Duas armadilhas do SINASC}\label{duas-armadilhas-do-sinasc}
...
Todo esse caminho está reunido em um script pronto: \href{https://ana-mat-br.github.io/scripts/dados-nascimentos.R}{\textbf{dados-nascimentos.R}}. Nós o percorreremos passo a passo na próxima seção. Para usá-lo direto, basta trocar duas linhas:
...
\section{Do DATASUS até o seu arquivo}\label{do-datasus-atuxe9-o-seu-arquivo}

Até aqui vimos os comandos soltos. Agora vamos juntar tudo: baixar do DATASUS, organizar e \textbf{salvar em um arquivo} que você abre depois, no R ou no Excel, sem precisar baixar de novo.

É este o caminho que produziu o \texttt{nascimentos.csv} usado neste livro. Os blocos abaixo não são executados quando o livro é gerado, para não baixar 235 mil registros a cada vez, mas você pode copiá-los e rodar.

\subsection{Passo 1: baixar}\label{passo-1-baixar}
...
\FunctionTok{nrow}\NormalTok{(bruto)}
\CommentTok{\#\textgreater{} [1] 235063}
\end{Highlighting}
\end{Shaded}

São todos os nascimentos de Minas Gerais em 2022. O download leva alguns minutos.

\subsection{Passo 2: traduzir os códigos}\label{passo-2-traduzir-os-cuxf3digos}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rotulado }\OtherTok{\textless{}{-}} \FunctionTok{process\_sinasc}\NormalTok{(bruto)}

\CommentTok{\# antes: 1 e 2.  depois: Masculino e Feminino}
\FunctionTok{table}\NormalTok{(rotulado}\SpecialCharTok{$}\NormalTok{SEXO)}
\CommentTok{\#\textgreater{} Feminino Masculino}
\CommentTok{\#\textgreater{}   114237    120826}
\end{Highlighting}
\end{Shaded}

\subsection{Passo 3: ficar só com o seu município}\label{passo-3-ficar-suxf3-com-o-seu-municuxedpio}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# a coluna munResNome vem do process\_sinasc()}
\NormalTok{sel      }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(rotulado}\SpecialCharTok{$}\NormalTok{munResNome }\SpecialCharTok{==} \StringTok{"Uberaba"}\NormalTok{)}
\NormalTok{bruto    }\OtherTok{\textless{}{-}}\NormalTok{ bruto[sel, ]}
\NormalTok{rotulado }\OtherTok{\textless{}{-}}\NormalTok{ rotulado[sel, ]}

\FunctionTok{nrow}\NormalTok{(rotulado)}
\CommentTok{\#\textgreater{} [1] 3602}
\end{Highlighting}
\end{Shaded}

Troque \texttt{"Uberaba"} pelo nome do seu município. Se quiser o estado inteiro, é só pular este passo.

\subsection{Passo 4: montar a tabela com as variáveis que interessam}\label{passo-4-montar-a-tabela-com-as-variuxe1veis-que-interessam}

Aqui escolhemos as colunas, damos nomes em português e limpamos os códigos de ``ignorado'' que vimos há pouco. Repare que o \textbf{peso vem do banco bruto}, pelo motivo explicado na seção das armadilhas.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(dplyr)}

\NormalTok{nascimentos }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}
  \AttributeTok{peso\_g       =} \FunctionTok{as.numeric}\NormalTok{(bruto}\SpecialCharTok{$}\NormalTok{PESO),        }\CommentTok{\# do bruto: o rotulado zera}
  \AttributeTok{idade\_mae    =} \FunctionTok{as.numeric}\NormalTok{(bruto}\SpecialCharTok{$}\NormalTok{IDADEMAE),}
  \AttributeTok{sem\_gestacao =} \FunctionTok{as.numeric}\NormalTok{(bruto}\SpecialCharTok{$}\NormalTok{SEMAGESTAC),}
  \AttributeTok{consultas\_pn =} \FunctionTok{as.numeric}\NormalTok{(bruto}\SpecialCharTok{$}\NormalTok{CONSPRENAT),}
  \AttributeTok{apgar5       =} \FunctionTok{as.numeric}\NormalTok{(bruto}\SpecialCharTok{$}\NormalTok{APGAR5),}
  \AttributeTok{sexo         =}\NormalTok{ rotulado}\SpecialCharTok{$}\NormalTok{SEXO,}
  \AttributeTok{parto        =}\NormalTok{ rotulado}\SpecialCharTok{$}\NormalTok{PARTO,}
  \AttributeTok{escol\_mae    =}\NormalTok{ rotulado}\SpecialCharTok{$}\NormalTok{ESCMAE,}
  \AttributeTok{raca\_mae     =}\NormalTok{ rotulado}\SpecialCharTok{$}\NormalTok{RACACOR}
\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{mutate}\NormalTok{(   }\CommentTok{\# fora destas faixas é código de ignorado, então vira NA}
    \AttributeTok{peso\_g       =} \FunctionTok{ifelse}\NormalTok{(peso\_g       }\SpecialCharTok{\%in\%} \DecValTok{250}\SpecialCharTok{:}\DecValTok{7000}\NormalTok{, peso\_g,       }\ConstantTok{NA}\NormalTok{),}
    \AttributeTok{idade\_mae    =} \FunctionTok{ifelse}\NormalTok{(idade\_mae    }\SpecialCharTok{\%in\%} \DecValTok{10}\SpecialCharTok{:}\DecValTok{60}\NormalTok{,    idade\_mae,    }\ConstantTok{NA}\NormalTok{),}
    \AttributeTok{sem\_gestacao =} \FunctionTok{ifelse}\NormalTok{(sem\_gestacao }\SpecialCharTok{\%in\%} \DecValTok{20}\SpecialCharTok{:}\DecValTok{45}\NormalTok{,    sem\_gestacao, }\ConstantTok{NA}\NormalTok{),}
    \AttributeTok{consultas\_pn =} \FunctionTok{ifelse}\NormalTok{(consultas\_pn }\SpecialCharTok{\%in\%} \DecValTok{0}\SpecialCharTok{:}\DecValTok{40}\NormalTok{,     consultas\_pn, }\ConstantTok{NA}\NormalTok{),}
    \AttributeTok{apgar5       =} \FunctionTok{ifelse}\NormalTok{(apgar5       }\SpecialCharTok{\%in\%} \DecValTok{0}\SpecialCharTok{:}\DecValTok{10}\NormalTok{,     apgar5,       }\ConstantTok{NA}\NormalTok{)}
\NormalTok{  )}

\FunctionTok{dim}\NormalTok{(nascimentos)}
\CommentTok{\#\textgreater{} [1] 3602    9}
\end{Highlighting}
\end{Shaded}

\subsection{Passo 5: salvar o arquivo}\label{passo-5-salvar-o-arquivo}

Agora o que interessa: gravar em disco. Há dois formatos úteis, e vale salvar nos dois.

\textbf{Em CSV}, que é o formato universal, lido por qualquer programa:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(readr)}

\FunctionTok{write\_csv}\NormalTok{(nascimentos, }\StringTok{"nascimentos.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\textbf{Em Excel}, que é o que a maioria das pessoas do seu grupo de pesquisa vai querer abrir:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# install.packages("writexl")   se ainda não tiver}
\FunctionTok{library}\NormalTok{(writexl)}

\FunctionTok{write\_xlsx}\NormalTok{(nascimentos, }\StringTok{"nascimentos.xlsx"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

O \texttt{writexl} é a opção mais simples para gravar Excel: não exige Java nem nenhuma instalação fora do R.

\begin{quote}
\textbf{Onde os arquivos foram parar?} Na pasta do seu Projeto do RStudio, a mesma de que falamos no capítulo de importação. Eles aparecem na aba \textbf{Files}, no canto inferior direito da tela. Se você não estiver dentro de um Projeto, vão para a pasta de trabalho atual, que você descobre com \texttt{getwd()}.
\end{quote}

\subsection{Passo 6: ler de volta}\label{passo-6-ler-de-volta}
...
\begin{Highlighting}[]
\CommentTok{\# do CSV}
\NormalTok{nascimentos }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"nascimentos.csv"}\NormalTok{)}

\CommentTok{\# ou do Excel}
\FunctionTok{library}\NormalTok{(readxl)}
\NormalTok{nascimentos }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(}\StringTok{"nascimentos.xlsx"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
\textbf{Por que salvar em vez de baixar toda vez?} Por três motivos. É muito mais rápido. Funciona sem internet. E, principalmente, \textbf{congela os dados}: o DATASUS revisa suas bases periodicamente, então um download feito daqui a seis meses pode não dar exatamente os mesmos números. Guardar o arquivo com o qual você fez as análises é parte da reprodutibilidade da sua pesquisa --- assunto do capítulo anterior.
\end{quote}
...
\textbf{O grupo pequeno é quem manda.} Quando os grupos têm tamanhos diferentes, a capacidade de detectar diferenças fica limitada pelo menor deles. Não adianta ter 3.240 pessoas de um lado se do outro há 362: a comparação só enxerga o que 362 permitem enxergar.
...
1.801 e 1.801 (balanceado, mesmo total) & 3.602 & \textbf{54 g} \\
...
A primeira: \textbf{um estudo balanceado seria melhor}. Com as mesmas 3.602 pessoas, mas divididas ao meio, detectaríamos 54 gramas em vez de 89. É por isso que, quando você \emph{planeja} uma coleta e pode escolher quantas pessoas põe em cada grupo, grupos iguais rendem mais.

A segunda: \textbf{mesmo assim, não jogue dados fora para equilibrar}. Sortear 362 mães do grupo grande, só para ficar simétrico, pioraria tudo --- passaríamos a detectar apenas 120 gramas. As 3.240 ajudam, só não ajudam tanto quanto ajudariam se estivessem distribuídas de outro jeito.

A terceira: \textbf{crescer só o grupo grande resolve pouco}. Repare na última linha: mesmo com um milhão de mães no segundo grupo, a diferença detectável cai de 89 para 87 gramas. Com o grupo pequeno preso em 362, não há tamanho do outro lado que salve.

\begin{quote}
\textbf{E podíamos ter equilibrado?} Não. Este é um estudo \textbf{observacional}: nós não decidimos quantas mães adolescentes tiveram filhos em Uberaba em 2022, apenas registramos o que aconteceu. O desbalanceamento aqui é uma característica da população, não um defeito do estudo. Equilibrar grupos é uma decisão de \textbf{planejamento}, e só existe quando você monta os grupos, como em um experimento.
\end{quote}

\begin{quote}
\textbf{Um detalhe técnico útil:} grupos desiguais combinados com variâncias diferentes são a situação em que o teste t mais precisa da correção de Welch, aquela do argumento \texttt{var.equal} que vimos no capítulo de comparação entre dois grupos. Aqui as variâncias de fato diferem um pouco (teste F, p = 0,04), e por sorte o R já faz a coisa certa: o \texttt{t.test()} usa Welch por padrão. É por isso que a saída dirá \textbf{Welch Two Sample t-test}.
\end{quote}
