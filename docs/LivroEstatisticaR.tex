...
\begin{quote}
\textbf{Atenção:} este banco é \textbf{simulado}. Ele não vem de uma coleta real: foi gerado por computador com médias e desvios plausíveis para universitários brasileiros. O script que o gera está em \texttt{scripts/gera-bancos-simulados.R}. Usamos simulação aqui porque bancos reais com escalas psicométricas raramente são públicos, e porque as grandes pesquisas nacionais que teriam esses dados (como a PNS) exigem técnicas de amostragem complexa que estão além do nosso escopo.
\end{quote}
...
O script completo, comentado, está disponível junto com o material do livro, em \texttt{scripts/dados-nascimentos.R}. Ele é curto e você pode adaptá-lo trocando duas linhas:
