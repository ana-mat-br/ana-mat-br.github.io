...
A estatística está em tudo na verdade\ldots{} e para dizer uma coisa ``bem chique'': \textbf{a estatística é a base da Inteligência Artificial}.

Adivinha quem está por trás:

\begin{itemize}
\item
  🤖 dos famosos algoritmos das redes sociais
\item
  🎬 das sugestões de filmes e músicas que aparecem no seu \emph{streaming} favorito
\item
  🔍 do ranqueamento de busca realizado por meio do \emph{Google}?
\item
  💬 dos agentes de IA
\end{itemize}
...
\section{💡 Desafio para Você}\label{desafio-para-vocuxea}

Basta realizar uma busca com os termos \textbf{``estatística''} + \textbf{um campo do seu curso que você se interessa}, que você encontrará um artigo científico.

\begin{quote}
\textbf{E se você não encontrar, comece a escrever sobre o tema! } 🚀
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{📚 Periódicos Recomendados por Área}\label{periuxf3dicos-recomendados-por-uxe1rea}

Para facilitar sua pesquisa, aqui estão alguns periódicos científicos organizados por área de conhecimento:

\subsubsection{🏃‍♂️ Ciência dos Esportes}\label{ciuxeancia-dos-esportes}
...
\subsubsection{🏥 Medicina e Saúde}\label{medicina-e-sauxfade}
...
\subsubsection{🧠 Psicologia}\label{psicologia}
...
\subsection{🔎 Outras Ferramentas de Busca Acadêmica}\label{outras-ferramentas-de-busca-acaduxeamica}
...
\tightlist
\item
  📚 \textbf{\href{https://www.mendeley.com}{Mendeley}} - Gerenciador de referências e descoberta de artigos científicos
\item
  🌎 \textbf{\href{https://scielo.org}{SciELO}} - Scientific Electronic Library Online (foco em América Latina)
\item
  🔍 \textbf{\href{https://scholar.google.com}{Google Scholar}} - Busca abrangente em literatura acadêmica
\item
  🏥 \textbf{\href{https://pubmed.ncbi.nlm.nih.gov}{PubMed}} - Especializada em ciências biomédicas e da saúde
\end{itemize}

\textbf{💡 Dica de busca avançada:}
- Use \texttt{"aspas"} para termos exatos: \texttt{"análise\ estatística"}
- Combine com operadores: \texttt{estatística\ AND\ psicologia}
...
Quando olhamos os artigos acima, podemos ver que todos eles têm resultados \textbf{descritivos} e \textbf{inferenciais}:
...
\bottomrule\noalign{}
\endlastfoot
\textbf{📈 Estatística Descritiva} & Descrever os dados amostrados para uma dada análise \\
\textbf{🔬 Estatística Inferencial} & Inferir - tirar conclusões a partir dos dados amostrados \\
\end{longtable}
}
...
A estatística não é apenas matemática abstrata - é uma \textbf{ferramenta essencial} para transformar dados em conhecimento, independentemente da sua área de atuação!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{📋 Atividade 1: Explorando a Estatística na Pesquisa Científica}\label{atividade-1-explorando-a-estatuxedstica-na-pesquisa-cientuxedfica}

A metodologia científica é composta por etapas estruturadas que guiam o processo de investigação. \textbf{A estatística está presente em várias dessas etapas}, desde o planejamento da coleta de dados até a análise e interpretação dos resultados. Nesta atividade, você irá identificar como a estatística é aplicada em um estudo real da sua área de interesse.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🎯 Objetivo da Atividade}\label{objetivo-da-atividade}

\textbf{Busque um artigo científico do campo de seu interesse que utiliza a estatística.}
...
\subsection{📝 Roteiro de Análise}\label{roteiro-de-anuxe1lise}

\textbf{1️⃣ PROBLEMA DE PESQUISA - Qual é o principal objetivo da pesquisa?}

\begin{quote}
\emph{Na metodologia científica, toda pesquisa começa com uma pergunta ou problema a ser investigado.}
\end{quote}
...
\textbf{2️⃣ METODOLOGIA - Como a pesquisa foi conduzida?}

\begin{quote}
\emph{A metodologia descreve o ``caminho'' percorrido pelo pesquisador. É aqui que a estatística começa a aparecer no planejamento da pesquisa.}
\end{quote}

\textbf{Analise os seguintes aspectos:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{🔬 Tipo de pesquisa} & A pesquisa é qualitativa, quantitativa ou mista? \\
\textbf{👥 População e Amostra} & Qual é a população-alvo? Quantos participantes compõem a amostra? \\
\textbf{🧮 Cálculo Amostral} & Os autores justificam o tamanho da amostra? Foi realizado cálculo amostral? \\
\textbf{📊 Técnica de Amostragem} & Como os participantes foram selecionados? (aleatória, por conveniência, estratificada, etc.) \\
\textbf{📋 Instrumentos de Coleta} & Quais instrumentos foram utilizados? (questionários, testes, medições, observações, etc.) \\
\textbf{📈 Análise Estatística} & Quais técnicas estatísticas foram aplicadas para analisar os dados? \\
\textbf{💻 Software Utilizado} & Qual programa foi usado para as análises? (SPSS, R, Python, Excel, etc.) \\
\end{longtable}
}

\textbf{💭 Reflita:} \emph{Por que essas escolhas metodológicas são importantes para a validade da pesquisa?}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{3️⃣ RESULTADOS - O que é apresentado por meio de tabelas ou gráficos?}

\begin{quote}
\emph{A estatística descritiva organiza e resume os dados coletados, facilitando a visualização e interpretação dos resultados.}
\end{quote}
...
\textbf{💡 Dica:} \emph{Preste atenção nas legendas, títulos e notas de rodapé das tabelas e gráficos - eles contêm informações importantes! }

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{4️⃣ GLOSSÁRIO ESTATÍSTICO - Termos encontrados no artigo}

\begin{quote}
\emph{Familiarizar-se com a terminologia estatística é essencial para compreender e aplicar essas técnicas em suas próprias pesquisas.}
\end{quote}

\textbf{Crie uma lista dos termos e conceitos estatísticos mencionados no artigo.}

\textbf{Exemplos do que você pode encontrar:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{📏 Medidas Descritivas} & Média, mediana, moda, desvio-padrão, variância, percentis \\
\textbf{📊 Tipos de Variáveis} & Variável dependente, independente, qualitativa, quantitativa \\
\textbf{🔬 Testes de Hipótese} & Teste t, ANOVA, qui-quadrado, Mann-Whitney, Kruskal-Wallis \\
\textbf{📈 Análises de Relação} & Correlação de Pearson, Spearman, regressão linear, regressão logística \\
\textbf{🎯 Conceitos Inferenciais} & Intervalo de confiança, nível de significância, p-valor, poder do teste, tamanho do efeito \\
\textbf{📐 Outros} & Normalidade, homocedasticidade, distribuição, outliers \\
\end{longtable}
}

\textbf{✍️ Para cada termo identificado:}
...
\textbf{💬 Questão Bônus:} \emph{Com base no artigo analisado, que decisões metodológicas você faria diferente se fosse conduzir uma pesquisa semelhante?}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\chapter{Conceitos Fundamentais}\label{conceitos-fundamentais}

Antes de mergulharmos nas análises estatísticas, é fundamental compreender dois conceitos essenciais que formam a base de qualquer estudo quantitativo: \textbf{Estatística Descritiva} e \textbf{Estatística Inferencial}.

Pense assim: a Estatística Descritiva \textbf{organiza e resume} os dados que você coletou, enquanto a Estatística Inferencial \textbf{tira conclusões} sobre uma população maior a partir da amostra.
...
Imagine que você deseja avaliar a \textbf{saúde mental dos estudantes da UFTM}, coletando informações sobre:

\begin{itemize}
\item
  Níveis de \textbf{estresse}
\item
  Níveis de \textbf{ansiedade}
\item
  Presença de \textbf{depressão}
\item
  \textbf{Bem-estar geral}
\end{itemize}

\begin{quote}
\textbf{Como esses dados são coletados? }\\
Essas informações são obtidas por meio de \textbf{instrumentos psicométricos} - questionários e escalas científicas padronizadas que medem características psicológicas de forma objetiva e confiável. Esses instrumentos passam por rigorosos processos de validação estatística para garantir que realmente medem o que se propõem a medir.
\end{quote}

\textbf{Exemplos de instrumentos que podem ser aplicados:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Estresse} & \textbf{PSS-10} (Escala de Estresse Percebido) & Avalia o quanto situações da vida são percebidas como estressantes \\
\textbf{Ansiedade} & \textbf{BAI} (Inventário de Ansiedade de Beck) ou \textbf{GAD-7} & Medem sintomas de ansiedade em diferentes intensidades \\
\textbf{Depressão} & \textbf{BDI-II} (Inventário de Depressão de Beck) ou \textbf{PHQ-9} & Avaliam presença e gravidade de sintomas depressivos \\
\textbf{Bem-estar} & \textbf{Escala de Bem-Estar Subjetivo} ou \textbf{WHO-5} & Medem qualidade de vida e satisfação geral \\
\end{longtable}
}

💡 \textbf{Importante:} Embora muitos desses instrumentos possam ser aplicados por pesquisadores de diferentes áreas (não apenas psicólogos), a \textbf{interpretação clínica} e o \textbf{diagnóstico} devem ser realizados exclusivamente por profissionais habilitados em psicologia ou psiquiatria.

\subsubsection{🔧 O que a Estatística Descritiva faz com esses dados?}\label{o-que-a-estatuxedstica-descritiva-faz-com-esses-dados}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{1. Organizar dados} & Cria tabelas sistemáticas & Agrupar respostas por curso, período, faixa etária \\
\textbf{2. Calcular médias} & Resume os dados em um número & Média de estresse = 6,5 (escala 0-10) \\
\textbf{3. Criar gráficos} & Facilita a visualização & Gráfico de barras mostrando \% de alunos com ansiedade baixa/média/alta \\
\textbf{4. Medir variabilidade} & Mostra o quanto os dados variam & Desvio padrão alto = muita diferença entre os alunos \\
\textbf{5. Identificar padrões} & Encontra relações entre variáveis & Alunos que dormem menos têm mais estresse? \\
\end{longtable}
}

\subsubsection{💡 Por que isso é importante?}\label{por-que-isso-uxe9-importante}

Essas análises criam um \textbf{panorama claro} da saúde mental dos estudantes, fornecendo base para:
...
\subsection{🏃‍♂️ Exemplo 2: Prática de Atividades Físicas}\label{exemplo-2-pruxe1tica-de-atividades-fuxedsicas}

Agora, suponha que você quer entender \textbf{como os estudantes se exercitam}.

\subsubsection{📋 Dados que você pode coletar:}\label{dados-que-vocuxea-pode-coletar}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Frequência semanal}

  \begin{itemize}
  \tightlist
  \item
    Quantos alunos praticam atividades \textbf{mais de 3x por semana}?
  \item
    Resultado: 45\% dos estudantes
  \end{itemize}
\item
  \textbf{Tipos de atividade}

  \begin{itemize}
  \tightlist
  \item
    Use um \textbf{gráfico ou tabela} para mostrar a distribuição:
...
\item
  ...
  \item
    Use um \textbf{gráfico ou tabela} para comparar
  \end{itemize}
\end{enumerate}

\subsubsection{🎯 Aplicação prática:}\label{aplicauxe7uxe3o-pruxe1tica}
...
\begin{quote}
\textbf{O que é?}\\
A Estatística Inferencial usa dados de uma \textbf{amostra} (grupo menor) para tirar conclusões sobre a \textbf{população inteira}, com margem de confiança conhecida.
\end{quote}

\subsection{💭 Exemplo 1: Saúde Mental dos Estudantes}\label{exemplo-1-sauxfade-mental-dos-estudantes-1}

Você pesquisou \textbf{200 estudantes} (amostra) e quer saber sobre \textbf{todos os 6.900 estudantes} da UFTM (população).

\subsubsection{🎯 O que a Estatística Inferencial permite fazer?}\label{o-que-a-estatuxedstica-inferencial-permite-fazer}

\textbf{Situação 1: Estimar a média populacional}
...
Da amostra\ldots{}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Para a população\ldots{}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Média de ansiedade = 7,0 (escala 0-10) & Estimativa: ansiedade média de todos os estudantes está entre \textbf{6,5 e 7,5} \\
Baseado em 200 alunos & Com \textbf{95\% de confiança} \\
\end{longtable}
}

\textbf{Situação 2: Comparar grupos diferentes}

Pergunta: \emph{``Estudantes de Medicina têm mais ansiedade que estudantes de Engenharia?''}
...
\item
  Resultado do teste: Diferença é estatisticamente significativa (p \textless{} 0,05)
\item
  Conclusão: Sim, estudantes de Medicina apresentam níveis significativamente maiores de ansiedade
\end{itemize}

\subsubsection{✅ Vantagem:}\label{vantagem}

Você \textbf{não precisa pesquisar todos os 6.900 alunos} - com uma amostra bem planejada, pode fazer afirmações confiáveis sobre toda a universidade!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🏃‍♂️ Exemplo 2: Prática de Atividades Físicas}\label{exemplo-2-pruxe1tica-de-atividades-fuxedsicas-1}

Você pesquisou \textbf{300 estudantes} de diferentes cursos.

\subsubsection{🎯 Perguntas que a Estatística Inferencial responde:}\label{perguntas-que-a-estatuxedstica-inferencial-responde}

\textbf{1. Qual a proporção real de estudantes ativos? }
...
\textbf{2. Educação Física vs.~Medicina: quem se exercita mais?}
...
A \textbf{Teoria de Probabilidade} é a ponte entre Estatística Descritiva e Inferencial!

\subsection{🔗 Como funciona?}\label{como-funciona}
...
Com base nos dados descritivos, podemos calcular a \textbf{probabilidade} de:
...
Isso permite \textbf{prever} e \textbf{prevenir}, não apenas descrever!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{👥 População vs.~Amostra}\label{populauxe7uxe3o-vs.-amostra}

Entender essa diferença é \textbf{FUNDAMENTAL} para qualquer pesquisa!

\subsection{🌍 População}\label{populauxe7uxe3o}

\begin{quote}
É o \textbf{conjunto completo} de todos os indivíduos que você quer estudar.
\end{quote}
...
\tightlist
\item
  👨‍🎓 \textbf{6.900 estudantes} (dado: DRCA, dezembro/2024 - Incluindo: graduação, cursos técnicos e pós-graduação)
\end{itemize}

\subsection{🎯 Amostra}\label{amostra}

\begin{quote}
É um \textbf{grupo menor e representativo} selecionado da população.
\end{quote}
...
\begin{itemize}
\item
  👥 \textbf{364 estudantes} selecionados aleatoriamente
\item
  Representa toda a população com \textbf{95\% de confiança}
\item
  Erro máximo de \textbf{5\%}
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{📐 Exemplo Aplicado Completo}\label{exemplo-aplicado-completo}

Vamos juntar tudo em um exemplo real:

\subsubsection{🎯 Objetivo da Pesquisa}\label{objetivo-da-pesquisa}

Avaliar o \textbf{bem-estar emocional} dos estudantes da UFTM.

\subsubsection{📊 Etapa 1: Coleta de Dados (Estatística Descritiva)}\label{etapa-1-coleta-de-dados-estatuxedstica-descritiva}

\begin{itemize}
\tightlist
\item
  \textbf{População:} 6.900 estudantes
\item
  \textbf{Amostra:} 364 estudantes (seleção aleatória)
\item
  \textbf{Instrumento:} Questionário padronizado (escala 0-10)
\end{itemize}

\subsubsection{📈 Etapa 2: Análise Descritiva}\label{etapa-2-anuxe1lise-descritiva}
...
\subsubsection{🔬 Etapa 3: Inferência Estatística}\label{etapa-3-inferuxeancia-estatuxedstica}
...
\item
  ✅ Média estimada: 6,8
\item
  📏 Intervalo de confiança (IC95\%): 6,65 a 6,95
\item
  📐 Cálculo: 6,8 ± 1,96 × 1,5/√364 = 6,8 ± 0,15
\item
  🎯 \textbf{Interpretação:}
  Temos 95\% de confiança de que o bem-estar emocional médio de TODOS os estudantes da UFTM está entre 6,65 e 6,95.
\end{itemize}

\begin{quote}
⚠️ \textbf{Atenção a uma confusão comum:} a \emph{margem de erro de 5\%} usada no cálculo do tamanho da amostra é dada em \textbf{pontos percentuais de uma proporção} --- ela não é um percentual da média. O intervalo de confiança de uma \textbf{média} depende do desvio padrão da amostra, como na conta acima. São duas coisas diferentes.
\end{quote}

\subsubsection{💡 O que é IC95\%?}\label{o-que-uxe9-ic95}

Se você repetisse essa pesquisa \textbf{100 vezes} com diferentes amostras aleatórias de 364 estudantes, e construísse \textbf{um intervalo a cada vez}:

\begin{itemize}
\item
  ✅ Cerca de \textbf{95 desses intervalos} conteriam a verdadeira média de todos os estudantes
\item
  ❌ Cerca de \textbf{5 deles} não conteriam (erro amostral)
\end{itemize}

\begin{quote}
💡 \textbf{Repare na direção da afirmação:} o que varia de amostra para amostra é o \textbf{intervalo}, não a média populacional. A média de todos os estudantes é um número fixo e desconhecido: ou ela está dentro deste intervalo específico, ou não está. O que temos 95\% de confiança é no \textbf{procedimento} que gerou o intervalo.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{✨ Resumo Visual}\label{resumo-visual}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Estatística Descritiva} & Organiza e resume os dados & Quando você quer \textbf{descrever} o que coletou \\
\textbf{Estatística Inferencial} & Tira conclusões sobre a população & Quando você quer \textbf{generalizar} da amostra para todos \\
\textbf{População} & Todos que você quer estudar & Define o \textbf{alcance} da sua pesquisa \\
\textbf{Amostra} & Grupo menor representativo & Torna a pesquisa \textbf{viável} \\
\textbf{Probabilidade} & Calcula chances e incertezas & \textbf{Conecta} descritiva e inferencial \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{💭 Agora reflita:} \emph{Em uma pesquisa sobre hábitos alimentares dos estudantes, como você aplicaria esses conceitos?}
...
O cálculo do \textbf{tamanho da amostra} é uma etapa \textbf{fundamental} no planejamento de qualquer pesquisa científica. Ele determina quantos participantes precisamos incluir no estudo para que os resultados sejam:

\begin{itemize}
\tightlist
\item
  ✅ \textbf{Representativos} da população estudada
\item
  ✅ \textbf{Estatisticamente confiáveis}
\item
  ✅ \textbf{Precisos} o suficiente para responder à pergunta de pesquisa
\item
  ✅ \textbf{Viáveis} em termos de tempo e recursos
\end{itemize}

\subsection{💡 O Que Acontece se Erramos no Tamanho da Amostra?}\label{o-que-acontece-se-erramos-no-tamanho-da-amostra}
...
\begin{quote}
\textbf{Em resumo:} Calcular corretamente o tamanho da amostra garante que sua pesquisa seja \textbf{cientificamente válida} e \textbf{eticamente responsável}.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🔢 Tipos de Cálculo Amostral Segundo o Objetivo Estatístico}\label{tipos-de-cuxe1lculo-amostral-segundo-o-objetivo-estatuxedstico}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Estimar uma proporção} & Cálculo para proporção & Percentual de alunos com ansiedade & Margem de erro, nível de confiança, proporção estimada \\
\textbf{Estimar uma média} & Cálculo para média & Média de horas de estudo por dia & Desvio padrão, margem de erro, nível de confiança \\
\textbf{Comparar dois grupos} & Teste t ou ANOVA & Comparar estresse entre cursos & Tamanho do efeito, poder estatístico, nível de significância \\
\textbf{Testar correlação} & Cálculo para correlação & Relação entre sono e desempenho & Tamanho do efeito esperado, poder estatístico \\
\textbf{Estudos experimentais} & Ensaio clínico/experimento & Efeito de uma intervenção & Grupos controle/experimental, poder, efeito esperado \\
\end{longtable}
}

💭 \textbf{Importante:} Cada tipo de análise estatística requer uma fórmula específica para cálculo amostral!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{📊 Cálculo Para Estimar uma Proporção: Passo a Passo}\label{cuxe1lculo-para-estimar-uma-proporuxe7uxe3o-passo-a-passo}

Quando o \textbf{objetivo é estimar uma proporção} (por exemplo, ``Qual percentual de estudantes tem ansiedade?''), precisamos de algumas informações:

\subsection{🔑 Informações Necessárias}\label{informauxe7uxf5es-necessuxe1rias}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{1. Tamanho da População (N)} & Total de indivíduos que você quer estudar & 6.900 estudantes \\
\textbf{2. Margem de Erro (E)} & Quanto você aceita ``errar'' na estimativa & 5\% (0,05) \\
\textbf{3. Nível de Confiança} & Quão confiante você quer estar & 95\% (padrão) \\
\textbf{4. Proporção Estimada (p)} & Estimativa inicial do fenômeno & 50\% (0,5) = pior cenário \\
\end{longtable}
}
...
Para população \textbf{infinita} ou \textbf{muito grande}:
...
\item
  ...
  \begin{itemize}
  \item
    Para 95\% de confiança: \textbf{Z = 1,96}
  \item
    Para 99\% de confiança: \textbf{Z = 2,58}
  \end{itemize}
\item
...
\subsection{🧮 Exemplo Prático: Pesquisa na UFTM}\label{exemplo-pruxe1tico-pesquisa-na-uftm}

\textbf{Situação:} Queremos estimar a proporção de estudantes com sintomas de ansiedade.

\textbf{Dados:}

\begin{itemize}
\item
  População: \textbf{6.900 estudantes}
\item
  Margem de erro: \textbf{5\%} (queremos estar ``corretos'' com ±5\%)
\item
  Nível de confiança: \textbf{95\%}
\item
  Proporção estimada: \textbf{50\%} (não sabemos, então usamos o pior cenário)
\end{itemize}
...
\textbf{Resultado:} Precisamos de \textbf{384 estudantes} na amostra.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🔄 Ajuste Para População Finita}\label{ajuste-para-populauxe7uxe3o-finita}

Quando a população é ``pequena'' (geralmente N \textless{} 10.000), aplicamos uma \textbf{correção}:
...
\textbf{Resultado final:} Com a correção, precisamos de \textbf{364 estudantes}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{💭 Interpretando os Resultados}\label{interpretando-os-resultados}
...
\section{⚠️ Atenção: Diferentes Objetivos = Diferentes Cálculos!}\label{atenuxe7uxe3o-diferentes-objetivos-diferentes-cuxe1lculos}

Até agora, calculamos amostra apenas para \textbf{estimar proporção} (exemplo: ``Qual \% de alunos tem ansiedade?'').

Mas o cálculo \textbf{muda conforme seu objetivo}. Veja:

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🎯 Tipos de Cálculo Amostral}\label{tipos-de-cuxe1lculo-amostral}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Estimar PROPORÇÃO} & Quer saber \textbf{quantos \%} & ``Qual \% pratica exercícios?'' & Usa margem de erro e proporção estimada \\
\textbf{Estimar MÉDIA} & Quer saber \textbf{valor médio} & ``Quantas horas de sono em média?'' & Precisa do \textbf{desvio padrão} (variabilidade) \\
\textbf{Comparar GRUPOS} & Quer ver se há \textbf{diferença} & ``Medicina tem mais estresse que Ed. Física?'' & Precisa do \textbf{tamanho do efeito} esperado \\
\textbf{Testar CORRELAÇÃO} & Quer ver se há \textbf{relação} & ``Sono afeta o desempenho?'' & Precisa da \textbf{força da correlação} esperada \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🤔 E Quando Tenho VÁRIOS Objetivos?}\label{e-quando-tenho-vuxe1rios-objetivos}

\subsection{Regra Simples:}\label{regra-simples}

\begin{quote}
\textbf{Calcule CADA objetivo separadamente e use a MAIOR amostra!}
\end{quote}
...
\textbf{Por quê?} Porque 364 alunos são suficientes para \textbf{todos} os objetivos!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{💡 Dica Extra: Ajuste para Perdas}\label{dica-extra-ajuste-para-perdas}
...
\section{🛠️ Ferramentas Para Cálculo de Amostra}\label{ferramentas-para-cuxe1lculo-de-amostra}

Nosso foco aqui \textbf{não é fazer cálculos manualmente}, mas sim \textbf{compreender os conceitos} e saber \textbf{onde buscar ajuda}.

Hoje existem ferramentas gratuitas que fazem todo o trabalho para você!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{💻 Calculadoras Online (Gratuitas)}\label{calculadoras-online-gratuitas}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Calculadora USP Bauru} & Proporções e médias simples & ✅ Rápida e intuitiva✅ Em português✅ Não precisa instalar & \href{http://estatistica.bauru.usp.br/calculoamostral/}{Acessar} \\
\textbf{OpenEpi} & Estudos em saúde e epidemiologia & ✅ Diversos tipos de cálculo✅ Estudos caso-controle, coorte✅ Interface simples & \href{https://www.openepi.com/SampleSize/SSMean.htm}{Acessar} \\
\textbf{G*Power} & Comparações e testes complexos & ✅ Muito completo✅ Padrão em pesquisas✅ Calcula poder estatístico & \href{https://www.gpower.hhu.de/en.html}{Download} \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{📊 No Software R (Para Quem Já Usa)}\label{no-software-r-para-quem-juxe1-usa}

\textbf{A ideia aqui é simples:} em vez de calcular manualmente usando fórmulas matemáticas, usamos \textbf{funções prontas} que fazem tudo para nós!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{\texorpdfstring{\textbf{Pacote \texttt{samplingbook}} - Cálculo de Amostra para Proporção}{Pacote samplingbook - Cálculo de Amostra para Proporção}}\label{pacote-samplingbook---cuxe1lculo-de-amostra-para-proporuxe7uxe3o}
...
\textbf{Pronto!} Você precisa de \textbf{364 estudantes}
...
\subsubsection{\texorpdfstring{\textbf{Alternativa: Fórmula Direto no R}}{Alternativa: Fórmula Direto no R}}\label{alternativa-fuxf3rmula-direto-no-r}
...
\subsubsection{\texorpdfstring{✅ \textbf{Vantagens}}{✅ Vantagens}}\label{vantagens}
...
\subsection{🤖 Usando Inteligência Artificial}\label{usando-inteliguxeancia-artificial}

Você também pode pedir ajuda a \textbf{ChatGPT}, \textbf{Copilot} ou outras IAs!
...
\textbf{⚠️ Importante:} Sempre \textbf{valide} os resultados da IA usando uma das calculadoras acima!
...
\subsection{💡 Dica Prática}\label{dica-pruxe1tica}

\textbf{Não sabe qual usar?} Faça o seguinte:

1️⃣ Use uma \textbf{IA} para entender o cálculo\\
2️⃣ Valide em uma \textbf{calculadora online}\\
3️⃣ Documente qual ferramenta usou no seu projeto

\textbf{Exemplo de como citar:}

\begin{quote}
\emph{``O tamanho amostral foi calculado utilizando a Calculadora Amostral da USP Bauru (\url{http://estatistica.bauru.usp.br/calculoamostral/}), considerando população de 6.900 estudantes, margem de erro de 5\% e nível de confiança de 95\%.''}
\end{quote}
...
O cálculo do \textbf{tamanho da amostra} é essencial para garantir pesquisas \textbf{precisas e confiáveis}.

A boa notícia? \textbf{Você não precisa ser expert em matemática!}

Com as ferramentas certas, esse processo se torna simples e rápido, permitindo que você foque no que realmente importa: \textbf{planejar e executar uma pesquisa de qualidade}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{📝 Atividade 2: Análise Crítica do Artigo}\label{atividade-2-anuxe1lise-cruxedtica-do-artigo}

\textbf{Com base no artigo que você buscou na Atividade 1, responda:}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🔍 Análise Metodológica}\label{anuxe1lise-metodoluxf3gica}

\textbf{1. Os autores discutem o cálculo do tamanho da amostra?}

Investigue:

\begin{itemize}
\item
  ✅ Há explicação sobre \textbf{como} o tamanho foi determinado?
\item
  ✅ Mencionam \textbf{fórmula}, \textbf{software} ou \textbf{ferramenta} utilizada?
\item
  ✅ Apresentam os \textbf{parâmetros} (margem de erro, confiança)?
\end{itemize}

💭 \emph{Se não houver essa informação, isso é uma limitação do estudo?}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{2. A população da pesquisa foi claramente definida?}
...
\item
  📋 Há critérios de \textbf{inclusão e exclusão}?
\item
  🌍 O contexto está bem delimitado (local, período)?
\end{itemize}

💭 \emph{Uma população mal definida compromete a validade dos resultados.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{3. O tamanho da amostra utilizado foi adequado?}

Avalie criticamente:

\begin{itemize}
\item
  ✅ A amostra parece \textbf{grande o suficiente}?
\item
  ⚖️ É \textbf{representativa} da população?
\item
  ⚠️ Os autores discutem \textbf{limitações} relacionadas ao tamanho da amostra?
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🧮 Cálculo Prático}\label{cuxe1lculo-pruxe1tico}

\textbf{4. Refaça o cálculo do tamanho de amostra}
...
\textbf{Tamanho no artigo} & & \\
\textbf{Seu resultado} & & \\
\textbf{Coincidem?} & & ☐ Sim ☐ Não \\
\end{longtable}
}

💭 \emph{Se não coincidem, quais podem ser as razões?}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{⚖️ Aspectos Éticos}\label{aspectos-uxe9ticos}

\textbf{5. A pesquisa foi aprovada pelo Comitê de Ética?}

Procure no artigo:

\begin{itemize}
\item
  ✅ Há menção à aprovação do \textbf{CEP}?
\item
  📋 O \textbf{número do parecer} está informado?
\item
  📝 Há referência ao \textbf{TCLE} (Termo de Consentimento)?
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{6. Qual a importância de submeter ao Comitê de Ética?}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Proteção dos participantes} & Garante que os direitos sejam respeitados \\
\textbf{Consentimento informado} & Participação voluntária e esclarecida \\
\textbf{Confidencialidade} & Proteção dos dados pessoais \\
\textbf{Riscos e benefícios} & Avaliação ética da relação custo-benefício \\
\textbf{Integridade científica} & Credibilidade da pesquisa \\
\end{longtable}
}

\textbf{⚠️ Consequências de NÃO submeter:}
...
\begin{quote}
\textbf{Conheça o Comitê de Ética em Pesquisa (CEP) da UFTM}

🔗 \url{https://www.uftm.edu.br/comitesecomissoes/cep}
\end{quote}

\textbf{Você encontrará:}
...
\textbf{💬 Questão bônus:} \emph{Se você fosse revisor deste artigo, aprovaria a metodologia amostral? Justifique.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🧮 Exercício Prático: Calculadora USP Bauru}\label{exercuxedcio-pruxe1tico-calculadora-usp-bauru}

\subsection{🎯 Situação-Problema}\label{situauxe7uxe3o-problema}

Você é pesquisador(a) e quer investigar a \textbf{adesão ao uso de equipamentos de proteção individual (EPIs)} entre profissionais de enfermagem de um hospital universitário.

\textbf{Dados da instituição:}

\begin{itemize}
\item
  \textbf{População:} 450 profissionais de enfermagem (enfermeiros, técnicos e auxiliares)
\item
  \textbf{Estimativa inicial:} Estudos semelhantes mostram que cerca de \textbf{70\%} dos profissionais utilizam EPIs corretamente
\item
  \textbf{Parâmetros desejados:} 95\% de confiança e margem de erro de 5\%
\end{itemize}

\textbf{Pergunta:} Quantos profissionais você precisa incluir na sua amostra?

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{📝 Passo a Passo na Calculadora}\label{passo-a-passo-na-calculadora}

\textbf{1. Acesse a calculadora:}

\begin{itemize}
\item
  🔗 \url{http://estatistica.bauru.usp.br/calculoamostral/}
\item
  Clique em \textbf{``Cálculos''} no menu
\item
  Selecione \textbf{``1 - Intervalo de Confiança de uma Proporção''}
\end{itemize}

\textbf{2. Preencha os campos conforme a imagem:}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Nível de Confiança} & Deixe marcado 95\% & 🔵 95\% \\
\textbf{Erro (\%)} & Margem de erro desejada & \textbf{5} \\
\textbf{Proporção Estimada na População (\%)} & Uso correto de EPIs & \textbf{70} \\
\textbf{☑️ População finita} & ✅ Marque a caixa & \textbf{450} \\
\end{longtable}
}

\textbf{3. Clique em ``Calcular''}
...
\begin{quote}
\textbf{R:} \_\_\_\_\_\_\_ profissionais
\end{quote}

\textbf{b) Qual a diferença do tamanho da amostra considerando população finita e infinita?}

\begin{quote}
\textbf{R: sem considerar população finita (infinita):} \_\_\_\_\_\_\_ profissionais
\end{quote}

\begin{quote}
\textbf{R: considerando população finita (450):} \_\_\_\_\_\_\_ profissionais
\end{quote}

\begin{quote}
\textbf{Diferença:} \_\_\_\_\_\_\_ profissionais
\end{quote}

\textbf{c) Volte às configurações originais. Agora aumente o nível de confiança para 99\%. O que acontece?}

\begin{quote}
\textbf{R: com 99\% de confiança:} \_\_\_\_\_\_\_ profissionais
\end{quote}

\textbf{d) Retorne para 95\% de confiança. Agora diminua a margem de erro para 3\%. Qual o impacto?}

\begin{quote}
\textbf{R: com erro de 3\%:} \_\_\_\_\_\_\_ profissionais
\end{quote}

\textbf{e) Volte às configurações originais (95\% de confiança, erro 5\%, proporção 70\%). Agora use o campo ``Perda de elementos (\%)'' e considere 20\% de perdas (profissionais de férias, licença, recusa). Quantas amostras você deveria ter?}

\begin{quote}
\textbf{R: ajustado para perdas:} \_\_\_\_\_\_\_ profissionais
\end{quote}
...
\textbf{1. Viabilidade prática:}\\
Considerando que você precisa pesquisar \_\_\_\_\_ profissionais de um total de 450, isso representa aproximadamente \_\_\_\_\_\% da população. Isso é viável? Por quê?

\textbf{2. Comparação:}\\
Por que o tamanho da amostra é MENOR quando consideramos população finita (450) em vez de infinita?

\textbf{3. Decisões metodológicas:}\\
...
\subsection{💡 Desafio Extra: Mude o Contexto}\label{desafio-extra-mude-o-contexto}

Agora imagine um cenário diferente:

\textbf{Situação:} Você não tem ideia de qual a proporção de profissionais que usam EPIs corretamente.

\textbf{O que fazer?} Quando não temos estimativa prévia, usamos \textbf{50\%} (pior cenário, que gera maior amostra).

\textbf{Teste:} Volte à calculadora, confira que os parâmetros estão nas configurações originais (95\% de confiança, erro 5\%, população finita de 450) e mude apenas a ``Proporção Estimada'' para \textbf{50\%}. Recalcule.

\textbf{Qual o novo tamanho da amostra?} \_\_\_\_\_\_\_ profissionais

\textbf{Por que mudou?} \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
...
\textbf{População finita: 450 profissionais \textbar{} Proporção: 70\%}

\textbf{a) Amostra necessária (erro 5\%, conf 95\%):} 189 profissionais
...
\textbf{e) Ajustado para 20\% de perdas:}

\begin{itemize}
\tightlist
\item
  pop. finita: 237 profissionais (189 ÷ 0,80 = 236,25, arredondando para cima)
\end{itemize}

\textbf{Desafio Extra (proporção 50\%):}

\begin{itemize}
\tightlist
\item
  pop. finita: 208 profissionais (contra 189 com proporção de 70\%)
\item
...
Pense assim: quando calculamos o tamanho da amostra para proporção, usamos a fórmula que inclui \textbf{p × (1 - p)}, onde \textbf{p} é a proporção estimada.
...
\bottomrule\noalign{}
\endlastfoot
10\% (0,10) & 0,10 × 0,90 = \textbf{0,09} & Baixa \\
30\% (0,30) & 0,30 × 0,70 = \textbf{0,21} & Média \\
\textbf{50\% (0,50)} & \textbf{0,50 × 0,50 = 0,25} & \textbf{MÁXIMA} ✅ \\
70\% (0,70) & 0,70 × 0,30 = \textbf{0,21} & Média \\
90\% (0,90) & 0,90 × 0,10 = \textbf{0,09} & Baixa \\
\end{longtable}
}

\textbf{📊 Observe:} O valor \textbf{0,25 (quando p = 50\%)} é o \textbf{MAIOR POSSÍVEL!}

\textbf{Reflexão 1:} 189/450 = 42\% da população - é uma amostra grande, mas viável em um hospital.

\textbf{Reflexão 2:} A correção para população finita considera que estamos pegando uma fração significativa do total (42\%), então precisamos de menos pessoas para ter precisão.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{🎯 O Que Você Aprendeu}\label{o-que-vocuxea-aprendeu}
...
\textbf{💬 Próximo passo:} Use esses conhecimentos para calcular a amostra da SUA pesquisa!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\chapter{Técnicas de Amostragem}\label{tuxe9cnicas-de-amostragem}

\section{🎯 Por Que Isso Importa?}\label{por-que-isso-importa}
...
\textbf{🎯 Objetivo principal:} Garantir que a amostra \textbf{represente bem} a população e \textbf{evite viés}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{⚠️ O Que é Viés de Amostragem?}\label{o-que-uxe9-viuxe9s-de-amostragem}

\textbf{Viés} acontece quando certos grupos têm \textbf{mais chance} de ser selecionados do que outros, distorcendo os resultados.

\subsection{📊 Exemplo Visual de Viés:}\label{exemplo-visual-de-viuxe9s}
...
\textbf{💡 Nossa missão:} Usar técnicas que \textbf{minimizem esse viés}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{📚 Tipos de Amostragem}\label{tipos-de-amostragem}

\subsection{Amostragens PROBABILÍSTICAS (Com Sorteio)}\label{amostragens-probabiluxedsticas-com-sorteio}

Todos têm chance \textbf{conhecida e não-zero} de ser selecionados.

\subsection{Amostragens NÃO PROBABILÍSTICAS (Sem Sorteio)}\label{amostragens-nuxe3o-probabiluxedsticas-sem-sorteio}

Seleção baseada em \textbf{conveniência, julgamento ou cotas}.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🎲 1. Amostragem Aleatória Simples}\label{amostragem-aleatuxf3ria-simples}

\begin{quote}
\textbf{Conceito:} Todos têm a \textbf{mesma chance} de ser escolhidos (como um sorteio de loteria).
\end{quote}
...
\subsection{💻 Ferramentas Práticas:}\label{ferramentas-pruxe1ticas}

\begin{itemize}
\item
  \textbf{No R:} Pacote base com função \texttt{sample()}
\item
  \textbf{No Excel:} \texttt{=ALEATÓRIOENTRE(1;N)}
\item
  \textbf{Online:} Random.org, calculadoras de números aleatórios
\item
  \textbf{Aplicativos:} Sorteador de nomes/números
\end{itemize}

\subsection{🎯 Exemplo Real - UFTM:}\label{exemplo-real---uftm}

\begin{quote}
\textbf{Pesquisa:} Avaliar saúde mental dos estudantes da UFTM
\end{quote}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Solicitar à secretaria a lista de \textbf{todos os alunos matriculados}
\item
  Sortear aleatoriamente \textbf{364 alunos} usando ferramenta digital
\item
...
\subsection{🛡️ Como Evitar Viés:}\label{como-evitar-viuxe9s}

\begin{itemize}
\item
  ✅ Garantir que a lista esteja \textbf{completa e atualizada}
\item
  ✅ Usar \textbf{geradores aleatórios confiáveis} (não escolher ``de cabeça'')
\item
  ✅ \textbf{Não substituir} sorteados por conveniência
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{📊 2. Amostragem Estratificada}\label{amostragem-estratificada}

\begin{quote}
\textbf{Conceito:} Dividir a população em \textbf{grupos homogêneos} (estratos) e sortear dentro de cada grupo proporcionalmente.
\end{quote}

\subsection{🎯 Quando Usar:}\label{quando-usar}

Use quando sua população tem \textbf{subgrupos importantes} que precisam estar representados.

\subsection{📋 Como Fazer - Exemplo Prático UFTM:}\label{como-fazer---exemplo-pruxe1tico-uftm}

\textbf{População:} Estudantes de graduação da UFTM\\
\textbf{Amostra necessária:} 364 estudantes

\begin{quote}
📌 Nos exemplos de amostragem a seguir trabalhamos apenas com a \textbf{graduação} (\textasciitilde6.750 alunos). O N de 6.900 usado no cálculo amostral incluía também pós-graduação e cursos técnicos. Como os dois números são próximos, a amostra de 364 continua adequada --- mas, na sua pesquisa, defina uma população só e use a mesma em todas as etapas.
\end{quote}

\textbf{Passo 1 - Definir estratos (por curso):}
...
\textbf{Passo 2 - Sortear aleatoriamente dentro de cada curso}
...
\tightlist
\item
  ✅ \textbf{Maior representatividade} de todos os cursos
\item
...
\subsection{🛡️ Como Evitar Viés:}\label{como-evitar-viuxe9s-1}

\begin{itemize}
\tightlist
\item
  ✅ Estratos devem ser \textbf{relevantes} para a pesquisa
\item
  ✅ Garantir que estratos sejam \textbf{mutuamente exclusivos} (aluno em apenas um curso)
\item
  ✅ Usar \textbf{alocação proporcional} (reflete a distribuição real)
\end{itemize}

\subsection{💡 Outras Formas de Estratificar na UFTM:}\label{outras-formas-de-estratificar-na-uftm}

\begin{itemize}
\tightlist
\item
  \textbf{Por período:} Calouros, intermediários, formandos
\item
  \textbf{Por campus:} Uberaba, Iturama
\item
  \textbf{Por turno:} Matutino, vespertino, noturno, integral
\item
  \textbf{Por área:} Saúde, Exatas, Humanas
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🔢 3. Amostragem Sistemática}\label{amostragem-sistemuxe1tica}

\begin{quote}
\textbf{Conceito:} Escolher um ponto de partida aleatório e selecionar a cada \textbf{k} elementos.
\end{quote}
...
\textbf{População:} 6.750 alunos\\
\textbf{Amostra necessária:} 364 alunos

\textbf{Passo 1 - Calcular o intervalo (k):}
...
\begin{quote}
⚠️ \textbf{Atenção ao arredondamento:} aqui o k deve ser arredondado \textbf{para baixo}. Se usássemos k = 19, percorrendo a lista de 19 em 19 sairiam apenas 6.750 ÷ 19 = 355 pessoas --- menos do que os 364 necessários. Com k = 18 saem 375, que cobre a amostra com folga.
\end{quote}

\textbf{Passo 2 - Sortear o primeiro (entre 1 e 18):}

\begin{verbatim}
Sorteio: número 7
\end{verbatim}

\textbf{Passo 3 - Selecionar a cada 18:}

\begin{verbatim}
Selecionados: 7, 25, 43, 61, 79, 97, 115, 133...
(sempre somando 18 até percorrer toda a lista)
\end{verbatim}
...
\tightlist
\item
  \textbf{No R:} Função \texttt{seq()} para criar sequência
\item
  \textbf{No Excel:} Fórmula com sequência aritmética
\item
  \textbf{Manual:} Calculadora + lista numerada
\end{itemize}

\subsection{🎯 Exemplo Prático:}\label{exemplo-pruxe1tico-1}

\begin{quote}
\textbf{Pesquisa na fila de vacinação:}
\end{quote}
...
\item
  Decide entrevistar \textbf{1 a cada 10 pessoas} da fila
\item
  Sorteia o início: 3ª pessoa
\item
  Depois: 13ª, 23ª, 33ª, 43ª\ldots{}
\end{itemize}
...
\tightlist
\item
  ✅ \textbf{Muito fácil} de aplicar no campo
\item
...
\tightlist
\item
  ❌ Se houver \textbf{padrão periódico} na lista, pode gerar viés
\item
  ❌ Menos aleatória que a amostragem simples
\end{itemize}

\subsection{🛡️ Como Evitar Viés:}\label{como-evitar-viuxe9s-2}

\begin{itemize}
\tightlist
\item
  ✅ Certifique-se que a lista \textbf{não tem ordem padrão}
\item
  ✅ Se houver ordem (ex: por curso), \textbf{embaralhe} antes
\item
  ✅ O primeiro elemento deve ser \textbf{verdadeiramente aleatório}
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🎪 4. Amostragem por Conglomerados}\label{amostragem-por-conglomerados}

\begin{quote}
\textbf{Conceito:} Dividir a população em \textbf{grupos} (conglomerados) e sortear \textbf{grupos inteiros}.
\end{quote}

\subsection{📋 Como Fazer - Exemplo UFTM:}\label{como-fazer---exemplo-uftm}

\textbf{Objetivo:} Pesquisar hábitos de estudo

\textbf{Opção 1 - Conglomerados = Turmas:}
...
\item
  Pesquisar \textbf{todos os alunos} dessas turmas
\end{itemize}

\textbf{Opção 2 - Conglomerados = Cursos:}
...
\textbf{Opção 3 - Conglomerados = Campus:}
...
\subsection{🎯 Exemplo Prático Nacional:}\label{exemplo-pruxe1tico-nacional}

\begin{quote}
\textbf{Pesquisa sobre educação no Brasil:}
...
\tightlist
\item
  ✅ \textbf{Muito prático} para populações grandes e dispersas
\item
  ✅ \textbf{Economiza tempo e custo} (concentra coleta)
\item
...
\tightlist
\item
  ❌ \textbf{Menos precisa} que outras técnicas
\item
  ❌ Conglomerados podem ser muito diferentes entre si
\item
  ❌ Aumenta o \textbf{erro amostral}
\end{itemize}

\subsection{🛡️ Como Evitar Viés:}\label{como-evitar-viuxe9s-3}

\begin{itemize}
\tightlist
\item
  ✅ Conglomerados devem ser \textbf{heterogêneos internamente} (diversos)
\item
  ✅ Conglomerados devem ser \textbf{semelhantes entre si}
\item
  ✅ Sortear \textbf{número suficiente} de conglomerados
\end{itemize}

\subsection{💡 Diferença: Estratificada vs.~Conglomerados}\label{diferenuxe7a-estratificada-vs.-conglomerados}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Divisão} & Grupos homogêneos & Grupos heterogêneos \\
\textbf{Seleção} & Amostra de \textbf{cada} grupo & \textbf{Alguns} grupos inteiros \\
\textbf{Objetivo} & Representar subgrupos & Facilitar coleta \\
\textbf{Exemplo UFTM} & Sortear alunos de cada curso & Sortear turmas inteiras \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{🚀 5. Amostragem por Conveniência}\label{amostragem-por-conveniuxeancia}

\begin{quote}
\textbf{Conceito:} Selecionar quem está \textbf{mais acessível} ao pesquisador.
\end{quote}
...
\tightlist
\item
  ❌ \textbf{Não probabilística} (não há sorteio)
\item
  ⚠️ \textbf{Alto risco de viés}
\item
  ✅ Rápida, prática e econômica
\end{itemize}

\subsection{📋 Exemplos Comuns:}\label{exemplos-comuns}

\textbf{1. Google Forms/Formulários Online:}

\begin{verbatim}
Pesquisador → Compartilha link → Quem quiser responde
\end{verbatim}

\textbf{2. Abordagem em locais específicos:}
...
\subsection{⚠️ Vieses Comuns em Formulários Online:}\label{vieses-comuns-em-formuluxe1rios-online}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Viés de acesso} & Só quem tem internet responde & Exclui quem não tem smartphone/dados \\
\textbf{Viés de auto-seleção} & Só os interessados respondem & Quem gosta do tema participa mais \\
\textbf{Viés de distribuição} & Enviado só para certos grupos & Só grupo de WhatsApp do seu curso \\
\textbf{Viés de plataforma} & Cada rede tem seu perfil & Instagram ≠ E-mail institucional \\
\end{longtable}
}
...
\begin{verbatim}
POPULAÇÃO REAL (20 cursos da UFTM):
🏥💼🎓🔬🏗️📚🎨🧪 (todos os cursos)
...
\subsection{✅ Quando Pode Ser Usada:}\label{quando-pode-ser-usada}

\begin{itemize}
\tightlist
\item
  ✅ Estudos \textbf{exploratórios} (fase inicial)
\item
  ✅ Pré-testes de questionários
\item
  ✅ Pesquisas \textbf{qualitativas}
\item
  ✅ Quando recursos são \textbf{muito limitados}
\item
  ✅ TCC de graduação com prazos curtos
\end{itemize}

\subsection{❌ Quando NÃO Deve Ser Usada:}\label{quando-nuxe3o-deve-ser-usada}

\begin{itemize}
\tightlist
\item
  ❌ Pesquisas que precisam de \textbf{generalização}
\item
  ❌ Estudos \textbf{quantitativos rigorosos}
\item
  ❌ Dissertações e teses
\item
  ❌ Publicações científicas de \textbf{alto impacto}
\end{itemize}

\subsection{🛡️ Como Minimizar Viés:}\label{como-minimizar-viuxe9s}

✅ \textbf{Diversificar canais:}
...
✅ \textbf{Descrever limitações honestamente:}
...
✅ \textbf{Comparar com dados populacionais:}
...
✅ \textbf{Usar junto com outras técnicas:}

\begin{verbatim}
Online (conveniência) + Presencial estratificado
\end{verbatim}

\subsection{💡 Exemplo de Relato Honesto no Artigo:}\label{exemplo-de-relato-honesto-no-artigo}

\begin{quote}
\emph{``Utilizou-se amostragem por conveniência através de formulário online (Google Forms), divulgado por e-mail institucional, redes sociais e grupos de WhatsApp. Obtivemos 412 respostas de estudantes da UFTM. Reconhecemos que esta técnica apresenta limitações quanto à representatividade, especialmente viés de auto-seleção e acesso digital. A amostra apresentou maior proporção de estudantes da área da Saúde (65\%) comparado à distribuição real dos cursos da UFTM (aproximadamente 40\%), o que deve ser considerado na interpretação dos resultados.''}
\end{quote}
...
Na vida real, pesquisadores frequentemente \textbf{combinam} técnicas para melhorar a representatividade!

\subsection{🎯 Exemplos de Combinações:}\label{exemplos-de-combinauxe7uxf5es}

\textbf{Exemplo 1: Estratificada + Aleatória Simples}
...
\textbf{Exemplo 2: Conglomerados + Sistemática}
...
\textbf{Exemplo 3: Estratificada + Conveniência com Cotas}
...
\subsection{📊 Caso Real - Pesquisa UFTM:}\label{caso-real---pesquisa-uftm}

\textbf{Situação:} Pesquisar bem-estar dos estudantes\\
\textbf{Recursos:} Limitados\\
\textbf{Tempo:} 2 meses

\textbf{Estratégia combinada:}
...
\textbf{Resultado:} Amostra mais representativa que só online, mais viável que só presencial!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{📊 Tabela Comparativa Resumida}\label{tabela-comparativa-resumida}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Aleatória Simples} & Probabilística & Baixa & Alta & Médio & Lista completa disponível \\
\textbf{Estratificada} & Probabilística & Média & Muito Alta & Médio-Alto & Subgrupos importantes \\
\textbf{Sistemática} & Probabilística & Baixa & Alta* & Baixo & Lista sem padrões \\
\textbf{Conglomerados} & Probabilística & Média & Média & Baixo & População dispersa \\
\textbf{Conveniência} & Não probabilística & Muito Baixa & Baixa & Muito Baixo & Estudos exploratórios \\
\end{longtable}
}
...
\section{🎯 Como Escolher a Técnica Ideal?}\label{como-escolher-a-tuxe9cnica-ideal}

\subsection{🤔 Fluxograma de Decisão:}\label{fluxograma-de-decisuxe3o}
...
\subsection{📋 Checklist de Decisão:}\label{checklist-de-decisuxe3o}
...
\section{🛡️ Evitando Viés: Guia Rápido}\label{evitando-viuxe9s-guia-ruxe1pido}

\subsection{⚠️ Principais Fontes de Viés:}\label{principais-fontes-de-viuxe9s}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Seleção} & Alguns grupos têm mais chance & Usar sorteio verdadeiramente aleatório \\
\textbf{Auto-seleção} & Só motivados participam & Incentivar e facilitar participação de todos \\
\textbf{Não-resposta} & Quem não responde é diferente & Fazer follow-up, oferecer incentivos \\
\textbf{Acesso} & Parte não é alcançável & Combinar canais (online + presencial) \\
\textbf{Temporal} & Coletar só em certo período & Distribuir coleta em diferentes momentos \\
\end{longtable}
}

\subsection{✅ Checklist Anti-Viés:}\label{checklist-anti-viuxe9s}
...
\section{💡 Dicas Finais}\label{dicas-finais}

\subsection{✅ Boas Práticas:}\label{boas-pruxe1ticas}

\textbf{1. Documente tudo:}
...
\subsection{🔵 Amostragens Probabilísticas:}\label{amostragens-probabiluxedsticas}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Aleatória Simples} & Sortear direto da lista completa \\
\textbf{Estratificada} & Dividir em grupos e sortear de cada um \\
\textbf{Sistemática} & Escolher a cada ``k'' elementos \\
\textbf{Conglomerados} & Sortear grupos inteiros \\
\end{longtable}
}

\subsection{🔴 Amostragem Não Probabilística:}\label{amostragem-nuxe3o-probabiluxedstica}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Conveniência} & Pegar quem está acessível (Google Forms, etc.) \\
\end{longtable}
}

\subsection{🎯 Regra de Ouro:}\label{regra-de-ouro}

\begin{quote}
\textbf{Sempre que possível, use técnicas PROBABILÍSTICAS!}\\
Elas permitem generalização e são mais aceitas cientificamente.

Se usar conveniência, seja \textbf{honesto sobre as limitações}!
\end{quote}
...
Na prática, os dados que vamos analisar estarão armazenados em um \textbf{banco de dados}, um arquivo de banco de dados pode ser de diferentes tipos, por exemplo:
...
\tightlist
\item
  Clicando na opção \textbf{From Text (readr)}, no botão \textbf{browser} indique onde (no seu computador) está localizado o arquivo a ser importado. A seguinte tela será apresentada:
\end{enumerate}
...
\begin{quote}
⚠️ \textbf{Cuidado ao conferir:} por padrão o R fecha as classes \textbf{à direita}, isto é, os intervalos são (30, 40{]}, (40, 50{]}, e assim por diante. Por isso o valor 40 conta na classe \emph{30 - 40}, e não na classe \emph{40 - 50}. O único elemento da segunda classe é o 46.
\end{quote}
...
\begin{quote}
⚠️ \textbf{E qual é a probabilidade de uma mulher ter batimentos exatamente iguais a 70bpm?}
...
\begin{quote}
⚠️ Existem também testes que examinam apenas \textbf{um aspecto} da normalidade, como a assimetria ou a curtose isoladamente. Neles a hipótese nula é outra (por exemplo, ``a assimetria é igual a zero''), e um resultado não significativo não garante normalidade. Veremos um caso desses adiante.
\end{quote}
...
\begin{quote}
💡 Para ver o teste de Mauchly e os df sem correção, use \texttt{summary(modelo\_afex)}.
\end{quote}
...
\begin{quote}
⚠️ \textbf{Se você tentar refazer a conta na mão, atenção a um detalhe:} o \(\chi^2\) que entra na fórmula do V é o valor \textbf{sem} a correção de continuidade de Yates. O \texttt{chisq.test()} aplica Yates por padrão em tabelas 2x2 e devolveu 0,543 --- mas \(\sqrt{0{,}543/30} = 0{,}13\), e não 0,20.
\end{quote}
