...
Seguindo o fluxograma do capítulo de comparação entre dois grupos: são \textbf{dois grupos}, \textbf{não pareados} (cada bebê está em um grupo só) e a variável de resposta é \textbf{quantitativa}. Falta decidir entre o teste t e o de Wilcoxon, e para isso verificamos a normalidade.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(nortest)}
\FunctionTok{ad.test}\NormalTok{(nascimentos}\SpecialCharTok{$}\NormalTok{peso\_g)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Passo 5: fazer o teste}\label{passo-5-fazer-o-teste}
...
O valor de p é \textbf{0,43}, bem acima de 0,05. \textbf{Não rejeitamos a hipótese nula}: não encontramos diferença no peso ao nascer entre os dois grupos.

Repare também no intervalo de confiança da diferença, que vai de cerca de −35 a 82 gramas. Como ele \textbf{inclui o zero}, diz a mesma coisa que o valor de p, só que de outro jeito.

\subsection{Passo 6: o passo que quase todo mundo pula}\label{passo-6-o-passo-que-quase-todo-mundo-pula}

Um resultado não significativo levanta sempre a mesma dúvida: \emph{não existe diferença, ou o estudo era pequeno demais para enxergá-la?} Quem responde isso é o \textbf{poder do teste}, que vimos no capítulo do pacote \texttt{pwr}.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(pwr)}

\CommentTok{\# qual o menor efeito que este estudo detectaria, com 80\% de poder,}
\CommentTok{\# dados os tamanhos dos dois grupos?}
\FunctionTok{pwr.t2n.test}\NormalTok{(}\AttributeTok{n1 =} \DecValTok{362}\NormalTok{, }\AttributeTok{n2 =} \DecValTok{3240}\NormalTok{, }\AttributeTok{sig.level =} \FloatTok{0.05}\NormalTok{, }\AttributeTok{power =} \FloatTok{0.80}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
O menor tamanho de efeito detectável é \textbf{d = 0,155}. Como o desvio padrão do peso é de 573 gramas, isso equivale a uma diferença de cerca de \textbf{89 gramas}.

A diferença observada foi de 24 gramas, ou seja, um d de 0,04. Está bem abaixo do que o estudo conseguiria detectar e, mais importante, é \textbf{pequena demais para ter significado clínico}. A conclusão, portanto, não é ``faltou amostra'': é que a diferença, se existe, não tem relevância prática.
...
\subsection{Passo 7: a pergunta que os dados realmente respondem}\label{passo-7-a-pergunta-que-os-dados-realmente-respondem}

O resultado acima é honesto, mas parece decepcionante. Ele não encerra o assunto: ele muda a pergunta.

Se o peso ao nascer não difere, será que a \textbf{assistência recebida durante a gravidez} difere? O banco traz o número de consultas de pré-natal, então dá para verificar.
...
Repare no percurso. Partimos de uma hipótese que parecia evidente e os dados disseram \textbf{não}. Em vez de forçar o resultado ou de desistir, usamos o poder do teste para mostrar que aquele ``não'' era confiável. Aí perguntamos outra coisa, e encontramos uma diferença real, em um desfecho que talvez importe mais: o acesso ao cuidado.
