...
> ⚠️ **Atenção a uma confusão comum:** a *margem de erro de 5%* usada no cálculo do tamanho da amostra é dada em **pontos percentuais de uma proporção** — ela não é um percentual da média. O intervalo de confiança de uma **média** depende do desvio padrão da amostra, como na conta acima. São duas coisas diferentes.


#### 💡 O que é IC95%?

Se você repetisse essa pesquisa **100 vezes** com diferentes amostras aleatórias de 364 estudantes, e construísse **um intervalo a cada vez**:

- ✅ Cerca de **95 desses intervalos** conteriam a verdadeira média de todos os estudantes

- ❌ Cerca de **5 deles** não conteriam (erro amostral)

> 💡 **Repare na direção da afirmação:** o que varia de amostra para amostra é o **intervalo**, não a média populacional. A média de todos os estudantes é um número fixo e desconhecido: ou ela está dentro deste intervalo específico, ou não está. O que temos 95% de confiança é no **procedimento** que gerou o intervalo.
...
**b) Qual a diferença do tamanho da amostra considerando população finita e infinita?**  
...
**e) Volte às configurações originais (95% de confiança, erro 5%, proporção 70%). Agora use o campo "Perda de elementos (%)" e considere 20% de perdas (profissionais de férias, licença, recusa). Quantas amostras você deveria ter?**  
...
**Teste:** Volte à calculadora, confira que os parâmetros estão nas configurações originais (95% de confiança, erro 5%, população finita de 450) e mude apenas a "Proporção Estimada" para **50%**. Recalcule.
...
> ⚠️ **Atenção ao arredondamento:** aqui o k deve ser arredondado **para baixo**. Se usássemos k = 19, percorrendo a lista de 19 em 19 sairiam apenas 6.750 ÷ 19 = 355 pessoas — menos do que os 364 necessários. Com k = 18 saem 375, que cobre a amostra com folga.

**Passo 2 - Sortear o primeiro (entre 1 e 18):**
```
Sorteio: número 7
```

**Passo 3 - Selecionar a cada 18:**
```
